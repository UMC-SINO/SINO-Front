...
type CardInput = Omit<NSCardType, 'id'>;

const withIds = (items: CardInput[], startId: number): NSCardType[] =>
  items.map((item, idx) => ({ id: startId + idx, ...item }));

export const SIGNAL_CARDS: NSCardType[] = withIds(
  [
    {
      title: '네이버 거래량 급증 감지',
...
export const NOISE_CARDS: NSCardType[] = withIds(
  [
    {
      title: '장중 미세 변동',
